...
Game::Game(QWidget *parent, bool net_, QTcpSocket *server) :
    BaseNetwork(parent),
    networkGame(net_),
    server(server) {

    if (networkGame) {
        connect(server, &QTcpSocket::readyRead, this, &Game::slotReadClient);
        myTurn = false;
    }
...
void Game::HandleAction(const Action& action){
    if(action.name == "next turn"){
        ...
        if (networkGame) {
            myTurn = false;
            send_(server, NetworkPackage(1, TextMessage(), GameDataMessage()));///!!!!!!!!!!!!!!!
        }
        _map->TileTick();
        setStyleSheet("background-color: " + _players[_currentplayer]->GetColor() + ";");
    }
...
void Game::hasGotPack() {
    ...
    if (pack.text_.isDefined()) {
        qDebug() << pack.text_.getText();
        if (pack.text_.getText() == "Your turn") {
            _currentplayer = 0;
            myTurn = true;
        } else {
            _currentplayer = 1;
    ...
    if (pack.gameData_.isDefined()) {
        for (auto i: pack.gameData_.actions_) {
            HandleAction(i);
        }
    }
}
